...
> **FedKD note (F10 re-confirmation, 2026-07-17):** row above supersedes the retired pre-fix figure (20.80% peak) — that number was measured on the old SimpleCNN student and is not comparable to this run (`min_rank_frac=0.25`, width-0.5 MobileNetV2GN student). The starvation _mechanism_ is confirmed fixed on the current architecture (`mean_rank_retained` holds at 0.278 vs. a same-model A/B minitest's 3.7% unfixed baseline), but the run stays volatile (±10pp swings) and trails every other baseline by 15–27pp. See [ADR-0005](../../adr/0005-baseline-stack-membership.md) for the full verdict.

> **F13 note (2026-07-17, first MobileNetV2GN measurement):** FedDistill, CFD, and FedAvg+KD had zero prior runs on this architecture. **CFD collapses to near-chance (10%) at both α** — not a fluke. FedAvg+KD is weak at severe skew (17.28%, well below every other baseline) but recovers under moderate skew (see Sweep B). FedDistill lands mid-pack, below the quantization baselines but ahead of collapsed FedKD/CFD. See [ADR-0005](../../adr/0005-baseline-stack-membership.md) for the full verdict.
