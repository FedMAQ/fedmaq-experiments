...
| **8** | [`stacked-reg-sweep-7-15/`](file:///c:/Users/Quirora/Documents/GitHub/fedmaq-experiments/docs/experiments/stacked-reg-sweep-7-15/)             |   July 15   | Sweeps stacked FedProx parameter-space penalty ($\mu$) on top of best-known client-side KD regularization parameters.           | Sweep active ($\mu \in \{0.0, 0.001, 0.01, 0.1, 1.0\}$). Expected to mitigate the Capacity-Drift Paradox under severe statistical skew.                           |
...
- **Moderate Skew ($\alpha=1.0$)**: Client KD regularization is highly successful, boosting ResNet18GN performance to **63.65%** (target config) and **65.94%** (best config). The regularized ResNet18GN successfully **outperforms FedMAQ-Lite** (+0.37pp) and reduces the gap to uncompressed FedAvg to just **−1.63pp** while saving ~90% communication bytes.
- **Severe Skew ($\alpha=0.1$)**: Adding KD regularization achieves a **+4.40pp** boost over the sweep's baseline. However, absolute accuracy is capped by suboptimal SimpleCNN hyperparameter defaults (`entropy_weight=4.0`). Lowering the entropy weighting threshold (e.g., to `1.0` as in the baseline) is required to allow the high-capacity ResNet18GN model to vote on out-of-distribution public samples.
...
- **Next Steps**: Transition to **Phase 2 (Iso-architecture comparisons)** using the full-sized **FedMAQ** (ResNet18GN, ~11.17M params) variant.
