...
**Last updated**: 2026-08-07 (Manuscript sync pass 7 — chapter 5's setup prose, never audited before: §5.2.2 still ran the headline communication contrast on the bytes-to-target scalar Decision 83 displaced grid-wide, leaving the chapter with no slot for an iso-byte comparison and so unable to express Decision 86. Plus Configuration 4's retired Formulation 1 fallback and §5.7's still-open accuracy-floor placeholder. See § "Manuscript Sync". Before it, pass 6 — chapter 3's first broad pass: Formulation 2's declared domain excluded two of the three ablation arms chapter 4 calls nested in it, and `grad_norm_ema` was still described as shipping. See § "Manuscript Sync". Before it, pass 5 — every fired branch in chapter 4 now records its outcome without deleting the branch, Configuration 8's §6.2 bullet is withdrawn, the tag's placement and the ablation's position relative to it are disclosed, and the run counts land at 177/147/36. See § "Manuscript Sync". Before it, 2026-08-06: Table 4.1's provenance is reproducible from committed code — `baseline_tuning_margin()` replaces the analyser the Stage 1b matrix named but which filters `algorithm == "fedmaq"` and could never read its own runs — and the `pre-registration` tag moves from `951f96a` to `b6b17b9`, the earliest commit carrying all three of §4.3.1's artifacts correctly and the last before the ablation dispatched — Decisions 87, 88. Before it: One iso-byte scorer now serves the formulation study, the baseline table and the ablation; measured against it, **FedMAQ does not beat FedAvg at equal bytes** — −5.44pp paired at α=1.0, null at α=0.1, against 38–40% fewer bytes — Decision 86. Before it: Formulation 2 frozen, ablation arms re-expressed, degenerate recheck skipped — Decisions 84, 85. Before it: 2026-08-02 (Stage 3 (`pass3_freeze_confirm`) skipped as degenerate — its surviving-set arm would have been config-identical to unrefined — and the pre-registered empty-freeze branch executed directly: `conf/algorithm/fedmaq.yaml` now freezes `soft_voting`/`ema_student`/`grad_norm_ema` all `false`, Ablation Configuration 8 dropped from `conf/matrix/ablation.yaml` (confirmatory total 183→177), `docs/freeze/resolved_configs.yaml` regenerated, full suite 188 passed — Decision 80. The manuscript's chapter_6.tex §6.2 contribution bullet resting on Configuration 8 was out of scope from here at the time and was **withdrawn 2026-08-07** in the sibling repo (manuscript `8d744c3`). Before it: `pass2_factorial` completed 26/26 clean under the Decision 78 fix — the surviving set came back **empty**: none of the seven refinement cells clears the noise margin, `soft_voting` is the closest at +3.3pp against a 5.6pp margin, `grad_norm_ema` and `ema_student` both trend negative individually. Per-seed data checked (not just the aggregate) to rule out a corrupted run given σ's wide 5-seed CI; the spread is genuine, not an artifact — Decision 79. Before it: Stage 1.2's re-dispatch aborted again at tasks 12–14 — not Decision 77's trigger recurring, but a teardown gap: a timeout-killed run's Ray processes could survive `ray stop` on Linux with nothing to force them down, contaminating the next task's `ray.init()`. `kill_ray_processes()` now force-kills on non-Windows too, matching the Windows branch — Decision 78. Before it, 2026-08-01: Stage 1.2's first dispatch died on its first three tasks; `client_gpus: 0.5` was measured and exonerated, the real defect was partition-ID resolution with no retry, which now aborts loudly rather than failing at the first blip — Decision 77. Before it, pass 5, pre-dispatch: the three exploration matrices were writing every cell of a stage into one output directory — fixed with `variant:` and a guard test across all twelve matrices, Decision 76. Pass 4 before it: Stage 1c pulls the grid's FedAvg reference rows forward, the calendar moves every other confirmatory run behind the tag, DAdaQuant's ceiling is unmixed from FedPAQ's units, Chapter 6 learns its own failure branches, and the uniform-memory arm is reframed as the memory-blind condition it actually is — Decisions 71–75)
...
- **2026-08-07** (pass 7) — manuscript commit `ce896fd`. Chapter 5's *setup* prose,
  audited for the first time: pass 5 filed the whole chapter as blocked on runs, which
  is true of its results but not of its framing. The load-bearing find is §5.2.2, which
  instructed the headline communication contrast on cumulative-MB-to-target — the
  scalar Decision 83 displaced for the entire grid, not just the formulation study
  (`chapter_4.tex:275`). As written the chapter had **no slot for an iso-byte
  comparison anywhere**, so it could not have expressed Decision 86's own result.
  Curves are now primary, the minimum-common-budget scalar carries the head-to-head,
  bytes-to-target is reported beside them. Three smaller fixes: §5.2.4 read
  rounds-to-target as the thing that judges total bytes (it is a descriptor,
  `chapter_4.tex:280`); §5.4 still carried Configuration 4's Formulation 1 fallback as
  an open "if Formulation 3 won", retired at `chapter_4.tex:403/411`; §5.7's red
  accuracy-floor placeholder left the primary-contribution claim looking withdrawable,
  though `chapter_6.tex:34` had already resolved that branch correctly. Chapter 6 was
  grepped for the same superseded-criterion vocabulary and is clean. Note the shape of
  the §5.2.2 find for future passes: the defect was a *missing* slot, visible only by
  asking which section would house a known result, not by checking any sentence
  against the code.

- **2026-08-07** (pass 6) — manuscript commit `e600643`, experiments commit `da53786`.
  Chapter 3's first broad pass; pass 5 had only grepped it for `soft-voting|entropy`.
  The load-bearing find is a contradiction between chapters, not a stale value:
  §3.3.3 declared Formulation 2 over $\omega_1, \omega_2 > 0$, while §4.3.7 builds the
  ablation's leave-one-out on $(0.5,0.5)/(0.5,0)/(0,0.5)$ and calls those arms nested
  in the winner. Two of the three sat outside the declared domain, so the argument
  ...
  (Decision 79). The narrow grep could not see it, which is the argument for the broad
  pass. Same defect live in §5.2, where $T = 1.0$ was still defended by the
  quantization-aware weighting that does not ship — replaced with the capacity-gap
  ground, retired ground recorded as retired. `conf/algorithm/fedmaq.yaml` carried the
  identical error in its `temperature` comment, as a live instruction to future agents
  resting on a dead premise; corrected there too. Builds clean, 119 pages, no undefined
  refs. Chapters 5 and 6 keep every placeholder. **Not done, needs the user**:
  `docs-audit` on the manuscript repo — disabled for model invocation, and the right
  instrument for the cross-repo arithmetic (177/147/141/36/30) that keeps moving.
  §4.5's calendar remains stale by standing instruction.

- **2026-08-07** (pass 5) — manuscript commits `8d744c3`, `64118f7`, `eed0ed3`. Every pre-registered branch
  in chapter 4 now carries its realized outcome, appended rather than substituted:
  the branch text stays because the pre-registration is the evidence, and a paragraph
  whose condition failed keeps its text and gains its result. Landed: Table 4.1's two
  revised constants with a `\textsuperscript{f}` marker and a Note that now states
  every algorithm-specific value was *tested* on this grid (Decision 81); §4.3.2's
  falsified claim that FedProx `mu=0.01` "does not transfer here" rewritten against
  its own next paragraph, which predicted `mu` was the one skew-sensitive constant;
  the Decision 83 amendment disclosed in the body with all three grounds, the
  acknowledged risk, and the superseded rule left standing; **§4.4.2 and §4.4.3
  amended too** — they define bytes-to-target and rounds-to-target and are downstream
  of the same amendment, which no prior handoff had flagged; Formulation 2 frozen with
  the skew split reported as the finding §4.3.6 pre-registers it to be; the
  fallback-arm protocol retired and §4.3.7 rewritten around Decision 84's mirror-image
  argument, which strengthens the leave-one-out rather than repairing it; Configuration
  8 dropped, §6.2's bullet **withdrawn** (owed since Decision 80) and §5.5's
  instruction to price the refinement layer retired; §4.3.1 states that the tag records
  the freeze rather than constituting it, disclosing the 36 ablation runs preceding the
  tag as a claim *distinct* from the six FedAvg rows preceding the freeze (Decision 88
  — do not merge them, they are admissible on different grounds); §4.3.5's two-stage
  error control disclosed as untested rather than exercised, since nothing was promoted.
  Counts corrected 183→177, 153→147, ablation 42→36; §4.5's "147 that remain from the
  153" recomputed to **141**, not find-replaced — the old subtrahend collides with the
  new confirmatory total. Also fixed here: Decision 88 quoted a stale `e8a5f6a` for the
  tag, which is at `0dd7ef1`. Builds clean, 119 pages, no undefined refs. **Chapters 5
  and 6 keep their placeholders** — 105 of the 177 runs do not exist, and Decision 86
  must frame §5 when they do.
