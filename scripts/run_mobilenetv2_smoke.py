"""Process-isolated runner for MobileNetV2GN smoke tests.

Runs FedAvg, FedProx, and FedMAQ for 50 rounds each under Dirichlet values 0.1 and 1.0.
FedMAQ uses default hyperparameters, with student EMA disabled and client-side KD regularization enabled.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run MobileNetV2GN smoke tests for FedAvg, FedProx, and FedMAQ."
    )
    parser.add_argument(
        "--total_rounds",
        type=int,
    ...
    for het in ["dirichlet_alpha_0.1", "dirichlet_alpha_1.0"]:
        ...
        # 3. FedMAQ (No EMA, only KD regularization)
        runs.append(
            {
                "alg": "fedmaq",
                "het": het,
    ...
    print("=" * 60)
    print("MobileNetV2GN 50-Round Smoke Tests")
