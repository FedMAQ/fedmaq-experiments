...
# Generate a single unified timestamp for this FEMNIST sweep session
date_str = datetime.now().strftime("%Y-%m-%d")
time_str = datetime.now().strftime("%H-%M-%S")
output_dir_base = f"multirun/{date_str}/{time_str}-femnist"
...
for alg in algorithms:
    for seed in seeds:
        runs.append({"algorithm": alg, "seed": seed, "index": run_idx})
        run_idx += 1

print("==================================================")
print(f"FEMNIST Sweep Base Directory: {output_dir_base}")
...
for run in runs:
    ...
    print("\n==================================================")
    print(
        f"Job {run['index'] + 1}/{len(runs)} | Starting {run['algorithm']} on FEMNIST seed={run['seed']}"
    )
