...
1. **Pass 2 Exploration**: Run Pass 2 sweeps using `uv run python scripts/run_matrix.py --matrix pass2_explore` for capacity-EMA (on/off), grad-norm-smoothing isolation, and client-KD-reg.
...
5. **Declarative Matrix Runner**: Implemented (`scripts/run_matrix.py` + `conf/matrix/`, Decision 39).
6. **Seed Determinism**: Verified partition generation and client sampling seed invariants (`pytest tests/test_environment.py` passing 100%).
