...
5. **Archive pattern conformance**: confirm archived material lives under a per-directory `archive/` subfolder, not a top-level `docs/archive/`. Flag deviations.
6. **Dead links**: check relative links/paths in edited docs resolve to real files. Auto-fix obvious renames; flag ambiguous cases.
7. **Report**: summarize what was auto-fixed vs. what needs a human decision, in that order.
