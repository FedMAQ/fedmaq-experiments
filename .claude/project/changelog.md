...
### 2026-07-13 — CFD Chance-Level Bug: Partial Fix, Root Cause Superseded

Early diagnosis + partial parameter-loading fix for CFD's chance-level accuracy; hypothesis and fix were later superseded by the full structural root-cause (client-side 1-bit vote hard-commitment under tiny per-client data budgets) and the decision to drop CFD entirely. See [docs/DECISIONS.md](../../docs/DECISIONS.md) Decision 26 and [docs/audits/distillation-direction-audit.md](../../docs/audits/distillation-direction-audit.md) F15 for the full account.
