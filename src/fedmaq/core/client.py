...
class GenericClient(fl.client.NumPyClient):
    ...
    def fit(
        ...
        if alg_name == "fedmd":
            return self._fit_fedmd(parameters, config)
        elif alg_name == "fedkd":
            return self._fit_fedkd(parameters, config)
        else:
            return self._fit_standard(parameters, config)
    ...
    def _fit_standard(
        ...
        momentum = float(
            exp_config.get(
                "momentum", self.config.get("algorithm", {}).get("momentum", 0.9)
            )
        )
        ...
        self.model.train()
        optimizer = torch.optim.SGD(
            self.model.parameters(), lr=lr, weight_decay=weight_decay, momentum=momentum
        )
        criterion = nn.CrossEntropyLoss()
    ...
    def evaluate(
        ...
        alg_name = self.config.get("algorithm", {}).get("name", "")
        if alg_name == "fedmd":
            return self._evaluate_fedmd(parameters, config)
        else:
            return self._evaluate_standard(parameters, config)
    ...
    def _evaluate_fedmd(
        ...
        if model_path.exists():
            self.model.load_state_dict(torch.load(model_path, map_location=self.device))
        return self._evaluate_standard(parameters, config, load_parameters=False)
