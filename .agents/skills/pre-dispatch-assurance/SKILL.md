...
Treat the pipeline freeze as eligible only when the local smoke verifier and same-candidate GPU repeatability both pass with preserved provenance. Then guide the author through the non-overwriting envelope re-declaration procedure in `docs/freeze/pre_dispatch_smoke_gate.md`. Keep pipeline, evidence, and results freezes distinct.
